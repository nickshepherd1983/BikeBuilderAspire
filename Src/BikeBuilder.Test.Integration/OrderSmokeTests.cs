...
public class OrderSmokeTests(BikeBuilderAppFixture fixture)
{
  ...
  async Task RunScenarioAsync(IPage storePageRaw, IPage wasmPage, IPage notificationPage)
  {
    ...
    await Expect(store.CartItem(componentName)).ToBeVisibleAsync(new() { Timeout = 8000 });
    ...
    await Expect(store.CartItem(buildName)).ToBeVisibleAsync(new() { Timeout = 8000 });
    await Expect(store.CartTotal).ToBeVisibleAsync(new() { Timeout = 8000 });

    // Change of heart: drop the bike build again, leaving a single-item order.
    await store.RemoveItemAsync(buildName);
    await Expect(store.CartItem(buildName)).ToBeHiddenAsync(new() { Timeout = 8000 });
    ...
    await Expect(store.EmptyCartMessage).ToBeVisibleAsync(new() { Timeout = 8000 });
  }
}
